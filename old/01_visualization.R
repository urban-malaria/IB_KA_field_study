...
ib_w = st_read(file.path(DataDir, "kano_ibadan_shape_files", "Ibadan_metro_wards_shapes", "Ibadan_wards.shp")) %>% 
  filter(Urban == 'Yes')
...
#ggsave(paste0(SampDir, '/results/', Sys.Date(), '_urban_wards_ibadan.png'), ib_wm, width = 10, height =8)
...
ib_l = st_read(file.path(DataDir, "kano_ibadan_shape_files", "Ibadan_metro_LGAs_shapes", "Ibadan_metro_LGAs.shp")) %>% 
mutate(group = ifelse(LGAName %in% urban_LGA, 'urban', 'semi-urban'))


ib_lm=con_gplot(ib_l,quo(factor(group)), quo(LGAName))+
  scale_fill_manual(name='', values = c("seashell", "lightpink1"))
...
ib_lwm=con_gplot(ib_l,quo(factor(group)), quo(LGAName))+
  scale_fill_manual(name='', values = c("seashell", "lightpink1"))+ 
...
semiU_settle_ = semiU_settle %>% dplyr::select(WardName,type) %>%  group_by(WardName, type) %>%  summarise(number=n())

#semi-urban
settle_type_w = bar_fun(semiU_settle_, quo(type), quo(number), quo(type), scale_fill, 12, 12, 12, 12, 'Settlement type per neighborhood')+
  facet_wrap(~WardName)
#ggsave(paste0(SampDir, '/results/', Sys.Date(), '_settlement_types_semi_urban_wards_bars.pdf'),settle_type_w)
...
df_wl = df_wl %>%  dplyr::select(-c(LGACode.y, StateCode.y, Timestamp.y, AMAPCODE.y))
...
pop_df = ib_wp_ %>%  mutate(wrd_nm_x = ifelse(wrd_nm_x == 'Oranyan' & lga_cod == '31007', 'Oranyan_7', wrd_nm_x)) %>% 
  dplyr::select(WardName=wrd_nm_x, population_size=mean, group)

sf::st_geometry(pop_df) <- NULL

#prepare settlement data (convert settlement data to proportion)
settle_type = ib_w_block %>%  mutate(WardName = ifelse(WardName == 'Oranyan' & LGACode.x == '31007', 'Oranyan_7', WardName)) %>% 
  dplyr::select(WardName, settle_type=type, LGACode.x) %>%  group_by(WardName, settle_type) %>%  summarise(number=n())
st_geometry(settle_type) <- NULL
...
# -------------------------------------------------------------------------------------------------
### Since clustering didn't work for urban areas, we could try clustering using other variables 
## ------------------------------------------------------------------------------------------------
...
df_all = cbind(df, val_water, val_EVI,val_housing_15) %>% filter(group == 'urban') 
urban_wards = df_all$WardName
  
df_all =df_all %>%  dplyr::select(-c(WardName, group)) %>%  dplyr::select(-c(starts_with("settlement")))

BIC <- mclustBIC(df_all)
plot(BIC)
summary(BIC)


mod1 <- Mclust(df_all, x=BIC)
summary(mod1, parameters = TRUE)
plot(mod1, what = "classification")
...
#getting cluster classes for mapping 
urban_clusters=data.frame(mod1$classification) %>% rownames_to_column()
urban_w= df_all%>% rownames_to_column() 
urban_class = left_join(urban_w, urban_clusters)
urban_class = cbind(urban_wards, urban_class) %>%  dplyr::select(WardName = urban_wards,
                                                                 mod1.classification)
#make a map of urban clusters 
...
#ggsave(paste0(SampDir, '/results/', Sys.Date(), '_urban_clusters_Ibadan.pdf'),urban_map)

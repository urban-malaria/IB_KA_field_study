...
#summary_Date <-  df1  %>%
 filter ( `Complete_part1` =='Complete' , (`LGA of Address`=="Kano Municipal" | `LGA of Address` == "Tarauni" |`LGA of Address`=="Dala" |`LGA of Address`=="Nassarawa"|`LGA of Address`=="Gwale" |`LGA of Address` =="Fagge"), `Number of Pregnancy New`< 5, !(`Ward` %in% wardds) ) %>%
...
summary_lga_sel <-  df1  %>%
  ...
  select(`q124b: How many pregnancies have you had`,  Total_Completed_by_LGA )
...
lgplot1<-ggplot(summary_lga_sel,
  ...
  geom_bar(stat = "identity", fill="#334425")+
  geom_text(aes(label = paste0(sprintf("%.0f", `q124b: How many pregnancies have you had`)," Pregnancies")), nudge_y = 20, vjust = -1.9) +
  geom_text(aes(label = signif(Total_Completed_by_LGA)), nudge_y = 2, vjust = -0.5 ) +

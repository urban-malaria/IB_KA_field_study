...
kn_wall = st_read(file.path(DataDir, "kano_ibadan_shape_files", "kano_metro_wards_shapes", "Kano_metro_wards.shp"))%>%
filter(Urban == 'Yes')

Kn_wm=ggplot(kn_wall) +
...
kn_lall = st_read(file.path(DataDir, "kano_ibadan_shape_files", "Kano_metro_sixLGA_shapes", "Kano_metro_sixLGA_shapes.shp"))

kn_lmall=ggplot(kn_lall) +
  geom_sf(color=alpha('#8971B3', 0.5))+
  geom_text_repel(
    data = kn_lall,
...
df_wall_kn=st_join(kn_wall, kn_lall, join =st_within)

kn_wm_catall=ggplot(df_wall_kn) +
  geom_sf(color=alpha('#8971B3', 0.5))+
  geom_text_repel(
    data = df_wall_kn,
    aes(label =  WardName, geometry = geometry),color ='black',
  ...
  map_theme() + 
  #labs(title= "All LGAs and wards in Kano")+
  xlab("")+
  ylab("")
...
kn_wpall = st_read(file.path(DataDir, "nigeria_wardpop", "GRID3_ward_admin_pop.shp"))%>%
  filter(wrd_nm_x %in% kn_wall$WardName)%>%
  filter(stat_cd=='KN')%>%
  filter(urban=='1')%>%
  arrange(wrd_nm_x)

kn_wpmall = con_gplot(kn_wpall,quo(mean), quo(wrd_nm_x))+
  scale_fill_continuous(name='Ward population \n count, Kano', low="#fcf9e8", high="#bd8600", guide="colorbar",na.value="transparent")
...
val_popndens <- raster::extract(raster,kn_wall, buffer = buffer, fun = mean, df =TRUE)
val_popndens = val_popndens$gpw_v4_population_density_rev11_2020_1_deg
df_ward_kn_popden = cbind(kn_wall, val_popndens)

kn_w_popden = con_gplot(df_ward_kn_popden,quo( val_popndens), quo(WardName))+
  scale_fill_continuous(name='Ward population \n density, Ibadan', low="#b8e6bf", high="#00a32a", guide="colorbar",na.value="transparent")
...
kn_w_block=st_join(kn_wall, df, join =st_overlaps)
kn_bar_dat = kn_w_block
st_geometry(kn_bar_dat) <- NULL

kn_bar_overall = kn_bar_dat %>% dplyr::select(type) %>%  group_by(type) %>%  summarise(number=n())
...
## --------------------------------------------------------------
df__kn_inform <- geojson_read(file.path(DataDir, 'kano_ibadan_informality', "kano_ibadan_settlements.geojson"),  what = "sp")
inform_sf_kn <- st_as_sf(df__kn_inform) %>%  filter(regions == 'Kano')
df_kn_wall = kn_wall%>%dplyr::select(-c(StateCode, Timestamp, AMAPCODE))
kn_w_inform=st_join(df_kn_wall, inform_sf_kn, join =st_overlaps)%>% 
...
# all wards 
settle_type_w_kn = ggplot() + 
  geom_sf(data =kn_w_inform, mapping = aes(fill = mean_k)) +
...
st_geometry(kn_w_inform)<- NULL
pop_inform_kn = left_join(kn_wpall, kn_w_inform, by = c('wrd_nm_x' = 'WardName'))
dt_kn = bi_class(pop_inform_kn, x = mean, y = mean_k, style = "fisher", dim = 3)
map_kn <- ggplot() +
  geom_sf(data = dt_kn, mapping = aes(fill = bi_class), color = "white", size = 0.1, show.legend = FALSE) +
...
#------------------------------------------------------------------------------
files_kn = list.files(path = file.path(DataDir, "EVI_nigeria_2020"), pattern = ".tif", full.names = TRUE)
files_kn0= sapply(files_kn, raster, simplify = F)
EVI = files_kn0 %>%  map(~extract(., kn_wall, buffer = buffer, fun = mean, df =TRUE))
...
sf18_kn = st_read(file.path(DHS_data, "Downloads", "NG_2018_DHS_11072019_1720_86355/NGGE7BFL/NGGE7BFL.shp")) %>% 
  filter(ADM1NAME == 'KANO')

dhs_kn = read.csv(file.path(CsvDir, "all_DHS_variables_urban_malaria.csv"), header = T, sep = ',') %>%
  filter(shstate == 'kano', dhs_year == '2018') %>%  dplyr::select(v001, p_test, child_6_59_tested_malaria)

dhs_kano = left_join(sf18_kn, dhs_kn, by=c('DHSCLUST' = 'v001')) %>%drop_na(p_test)


##points_kn = st_join(dhs_kano, map_df_all_n, join = st_intersects)
...
--------------------------------------------------------------------------------
df_hf_kn_all <- geojson_read(file.path(DataDir, 'kano_health_facilities', "health-care-facilities-primary-secondary-and-tertiary.geojson"),  what = "sp")
head(df_hf_kn_all)
View(df_hf_ib)

df_sf_kn_all <- st_as_sf(df_hf_kn_all) 

view(df_sf_ib)

df_sf_kn_n_all = df_sf_kn_all %>% dplyr::select(c(id, latitude,longitude, category, global_id, name,ward_code, ward_name, lga_code, lga_name, geometry)) 

df_sf_ib_n0 = df_sf_ib_n %>% dplyr::filter(lga_name == 'Ibadan North')


df_sf_kn_nwall = st_join(df_wall_kn, df_sf_kn_n_all, join =st_intersects)
head(df_sf_kn_nwall)
kn_hf=ggplot(df_sf_kn_nwall) +
  geom_sf(color=alpha('#8971B3', 0.5))+
  geom_text_repel(
    data = df_sf_kn_nwall,
    aes(label =  name, geometry = geometry),color ='orchid',
    stat = "sf_coordinates", min.segment.length = 0, size = 2.0, force = 1)+
  map_theme() + 
  #labs(title= "HFs in Kano")+
  xlab("")+
  ylab("")
